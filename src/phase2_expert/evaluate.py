...
--out defaults to data_phase2_expert/<variant>_scores.csv, where <variant> is read back
from the CHECKPOINT's own saved config (model.variant_name()), same self-describing-
checkpoint convention as phase2/evaluate.py.

Usage:
    python src/phase2_expert/evaluate.py
    python src/phase2_expert/evaluate.py --checkpoint checkpoints/phase2_expert/camembert_experts.pt --split test
"""
...
DATA_PHASE2_EXPERT_DIR = Path(__file__).parent.parent.parent / "data" / "data_phase2_expert"
...
def main():
    ...
    parser.add_argument("--out", default=None, help="Output CSV path (default: data_phase2_expert/<variant>_scores.csv, variant read from the checkpoint itself)")
    ...
    out_path = Path(args.out) if args.out is not None else DATA_PHASE2_EXPERT_DIR / f"{model.variant_name()}_scores.csv"
    ...
    print("=== Done ===")
    print(f"\nTo compare against phase2's full camembert_mlp model, run:\n"
          f"  python src/phase2_expert/compare.py --experts-score {out_path}")
    print(f"\nTo see which expert(s) actually drove these scores (usage distribution + "
          f"pairwise similarity), run:\n"
          f"  python src/phase2_expert/analyze_experts.py --scores {out_path}")
