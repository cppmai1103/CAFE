...
plus ner_features.csv (see extract_ner_features.py) for the candidate spans.

Why ner_features.csv is a required input, not just train data: "context" is defined
relative to a candidate's span (start_token_id..end_token_id), which is not known ahead
of time from the train data alone -- it comes out of the NER extraction step. Same
reasoning as extract_ocr_features.py.
...
context_window_length (one row per NER candidate, in the same order as ner_features.csv).
...
(shouldn't happen in practice; see extract_ner_features.py) or when no context token on
that side was scoreable.
...
Usage:
    ...
    python extract_context_features.py
    python extract_context_features.py --ner-features /tmp/smoke_ner.csv --out /tmp/smoke_context.csv
"""
...
DEFAULT_OUT = DATA_DIR / "hipe2020_train_fr_context_features.csv"
...
def main():
    ...
    parser.add_argument(
        ...
        help="Train data CSV produced by preprocessing_data.py (with dictionary_score/sentence_ocr_mean)",
    )
    parser.add_argument(
        "--ner-features",
