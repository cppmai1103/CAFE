...
GLINER_MODEL_ID/LABELS there). predicted_entity_type in ner_features.csv is always in the
fixed HIPE scheme (LABELS), regardless of what prompt wording produced it -- --prompt-labels
here rebuilds the schema and must be given the SAME wording extract_ner_features.py was
run with (its own --prompt-labels), or field_names/type_idx below won't line up with the
model's actual schema. Both interpretation methods come from Captum (https://captum.ai/):
...
Usage:
    ...
    python src/feature_extraction/extract_attribution_features.py --method ig
    python src/feature_extraction/extract_attribution_features.py --method lime --n-samples 200
    python src/feature_extraction/extract_attribution_features.py --limit 20 --out /tmp/smoke_attribution.csv
"""
...
from gliner.extract_ner_features import (
...
def prepare_sentence(extractor: GLiNER2, sentence_text: str, prompt_labels: list[str], device: str) -> dict:
    ...
    GLiNER2 word's character span within sentence_text. prompt_labels must be the same
    wording extract_ner_features.py was run with (see --prompt-labels there), so
    field_names here lines up with predicted_entity_type after mapping it back through
    hipe_to_prompt."""
    schema = extractor.create_schema().entities(prompt_labels)
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Comma-separated labels to rebuild GLiNER2's entities schema with -- MUST match whatever "
            "extract_ner_features.py was run with (--prompt-labels there), one per LABELS entry "
            f"({', '.join(LABELS)}) in that order (default: {','.join(DEFAULT_PROMPT_LABELS)})"
        ),
    )
    ...
    hipe_to_prompt = dict(zip(LABELS, prompt_labels))
    ...
    for cand in tqdm(ner_df.to_dict("records"), total=len(ner_df), desc="Computing attribution features", unit="candidate"):
        ...
        if key != current_key:
            ...
            if sentence_text is None or not spans:
                ...
                skipped += 1
                continue
            prep = prepare_sentence(extractor, sentence_text, prompt_labels, device)
        ...
        prompt_label = hipe_to_prompt.get(predicted_type)
        if pd.isna(start_tid) or pd.isna(end_tid) or prompt_label not in prep["field_names"]:
            skipped += 1
            continue
        ...
        type_idx = prep["field_names"].index(prompt_label)
