...
deduplicate_ner_features.csv (see gliner/deduplicate_ner_features.py) for the candidate
spans, already resolved to pairwise non-overlapping per sentence.

Why a candidates CSV is a required input, not just train data: a candidate's span is a
range of train-data tokens (start_token_id..end_token_id), not known ahead of time from
the train data alone -- it comes out of the NER extraction (and deduplication) steps.

Pipeline:
    train data CSV (dictionary_score/sentence_ocr_mean/document_ocr_mean already computed
    by preprocessing_data.py) + deduplicate_ner_features.csv candidates -> for each
    candidate, aggregate its covering tokens' dictionary_score, skipping punctuation
    tokens entirely, and attach its sentence/document OCR means -> OCR features CSV.
...
NER candidate, in the same order as --ner-features). ocr_correct is True iff every
...
Usage:
    ...
    python src/feature_extraction/extract_ocr_features.py
    python src/feature_extraction/extract_ocr_features.py --ner-features /tmp/smoke_ner.csv --out /tmp/smoke_ocr.csv
"""
