...
column (assigned once, per document, by preprocessing_data.py's assign_splits).

Usage:
    python src/feature_extraction/prepare_data_logistic.py
    python src/feature_extraction/prepare_data_logistic.py --label-reliability data/label_reliability_span_type.csv --out data/logistic_regression_data.csv
"""
...
def main():
    ...
    parser.add_argument("--train-data", default=str(DEFAULT_TRAIN_DATA), help="Token-level train data CSV (for the document->split map)")
    ...
    print("=== Step 5: Save prepared logistic-regression data ===")
