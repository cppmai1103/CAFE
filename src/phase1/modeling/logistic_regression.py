...
Output:
    data_baseline/test_results/logistic_regression.csv -- document_id, sentence_id,
        ...
        same shape as platt_scaling.csv; the join-key + calibrated_score columns are
        exactly what modeling/plot_reliability_diagram.py's --logistic-score expects,
        split and ner_score are extra.
    ...
    checkpoints/baseline/logistic_regression.pt -- {"model", "scaler", "fit_stats",
        "feature_names"}, everything build_feature_matrix + scaler.transform +
        model.predict_proba need to reproduce calibrated_score on new candidates without
        refitting (see save_checkpoint()/load_model() below).

Usage:
    python src/phase1/modeling/logistic_regression.py
    python src/phase1/modeling/logistic_regression.py --data data/data_baseline/logistic_regression_data.csv --out data/data_baseline/test_results/logistic_regression.csv --figures-dir figures/modeling/train_tracking
"""
...
DEFAULT_OUT = TEST_RESULTS_DIR / "logistic_regression.csv"
...
DEFAULT_CHECKPOINT_OUT = CHECKPOINTS_DIR / "logistic_regression.pt"
...
def save_checkpoint(
    model: LogisticRegression, scaler: StandardScaler, fit_stats: dict, feature_names: list[str], path: str | Path,
) -> None:
    ...
    freshly-loaded model can't be fed a matching feature matrix. torch.save is used purely
    for a checkpoints/ layout/naming convention consistent with the Phase 2 models (see
    phase2_simple/model.py) -- torch's pickle-based serializer handles a plain dict of
    Python/sklearn objects fine, no tensor conversion needed since B3 has no PyTorch model
    of its own."""
    torch.save({"model": model, "scaler": scaler, "fit_stats": fit_stats, "feature_names": feature_names}, path)


def load_model(path: str | Path) -> tuple[LogisticRegression, StandardScaler, dict, list[str]]:
    checkpoint = torch.load(path, weights_only=False)
    return checkpoint["model"], checkpoint["scaler"], checkpoint["fit_stats"], checkpoint["feature_names"]
...
def main():
    ...
    print("=== Step 6: Save logistic_regression.csv to test_results/ (test split only) ===")
