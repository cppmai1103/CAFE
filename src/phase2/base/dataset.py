...
Usage (quality-check mode -- prints a decoded sample batch, doesn't train anything):
    python src/phase2/base/dataset.py
    python src/phase2/base/dataset.py --split train --batch-size 4 --print-batches 1
"""
...
from phase2.base.tokenize_windows import DEFAULT_ENCODER_NAME, DEFAULT_MAX_LENGTH, tokenize_candidate_window
...
class Phase2WindowDataset(Dataset):
    def __init__(
        ...
        (e.g. mid-migration to a new split scheme). entity_type_vocab: None (default)
        uses the project's standard HIPE-2022 5-type vocab; pass a custom dict (see
        phase2/vocab.py's entity_type_vocab_for) if the windows file's predicted_type
        values come from a different NER source's own tagset -- must match whatever
        vocab the paired Phase2Model was built with (see phase2/train.py's --labels)."""
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        return {
            ...
            "entity_type_id": self.entity_type_vocab[record["predicted_type"]],
