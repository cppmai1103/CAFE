...
output shape as src/phase1/modeling/platt_scaling.py/logistic_regression.py/mlp_baseline.py --
...
--out defaults to data_phase2/test_results/<variant>_scores.csv, where <variant> is read
back from the CHECKPOINT's own saved config (model.variant_name()) -- not re-derived from
CLI flags you'd have to remember to repeat -- so scoring an ablation checkpoint (see
train.py's --no-* flags) never collides with the full model's (or another ablation's)
scores CSV. Kept in its own test_results/ subfolder (name unchanged otherwise) since the
default --split is test-only -- same convention as
modeling/platt_scaling.py/logistic_regression.py/mlp_baseline.py's test_results/ output.

Usage:
    python src/phase2/base/evaluate.py
    python src/phase2/base/evaluate.py --checkpoint checkpoints/phase2/camembert_mlp.pt --split test
    python src/phase2/base/evaluate.py --checkpoint checkpoints/phase2/camembert_mlp_without_ner_score.pt  # writes data_phase2/test_results/camembert_mlp_without_ner_score_scores.csv
"""
...
from phase2.base.model import encoder_short_name, load_model, variant_name
from phase2.base.train import DEFAULT_CHECKPOINT_OUT

DATA_PHASE2_DIR = Path(__file__).parent.parent.parent.parent / "data" / "data_phase2"
TEST_RESULTS_DIR = DATA_PHASE2_DIR / "test_results"
...
def main():
    ...
    parser.add_argument("--out", default=None, help="Output CSV path (default: data_phase2/test_results/<variant>_scores.csv, variant read from the checkpoint itself)")
    ...
    # never a separate CLI flag here -- guarantees the dataset maps predicted_type
    # strings to the exact same ids this model was trained with, no train/eval mismatch
    # risk possible.
    ...
    if is_full_model:
        print(f"\nTo compare against B0/B1/B3/MLP and produce the standard plots, run:\n"
              f"  python src/phase1/modeling/plot_reliability_diagram.py --platt-scaling-score data/data_baseline/test_results/platt_scaling.csv "
              f"--logistic-score data/data_baseline/test_results/logistic_regression.csv --mlp-score data/data_baseline/test_results/mlp_baseline.csv "
              f"--camembert-mlp-score {out_path} --camembert-mlp-label {model.variant_name()}")
    else:
        full_model_label = variant_name(encoder_name=model.encoder_name)
        print(f"\nThis is an ablation variant -- to compare it against the full model, add it as an --extra-score, e.g.:\n"
              f"  python src/phase1/modeling/plot_reliability_diagram.py --camembert-mlp-score data_phase2/test_results/{full_model_label}_scores.csv "
              f"--camembert-mlp-label {full_model_label} --extra-score {model.variant_name()}={out_path} --figures-dir figures/ablation")
