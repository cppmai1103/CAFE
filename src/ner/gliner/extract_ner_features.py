...
docs/phase1_manual_features.md's baseline notes). --labels-file points at a JSON file of
{TYPE: prompt wording} (default: src/ner/gliner/labels.json, HIPE-2022's 5-type scheme --
see load_label_map()'s docstring); its keys become LABELS (what's actually written to
predicted_entity_type) and its values are what's sent to GLiNER2 as the prompt, zipped
positionally to build prompt_to_hipe so whatever the model returns under its own prompt
wording is remapped back to the fixed output scheme. Downstream files never see the
prompt wording -- only the output scheme. A different NER source with its own tagset
just points --labels-file at a different JSON file -- nothing in this script itself
needs to change.
...
Usage:
    ...
    python src/ner/gliner/extract_ner_features.py --limit 50 --out /tmp/smoke_test.csv  # quick smoke test
    python src/ner/gliner/extract_ner_features.py --labels-file data/data_source/letemps/letemps_fr_labels.json --load-data data/data_source/letemps/letemps_fr.csv --out data/letemps_fr/gliner/data_baseline/ner_features.csv
"""
...
def load_label_map(labels_file: str | Path) -> dict[str, str]:
    """Reads a {TYPE: prompt wording} JSON file, e.g. {"PERS": "person", "LOC":
    "location", ...} -- TYPE is the fixed output scheme predicted_entity_type is stored
    as in ner_features.csv, regardless of a corpus's own annotation wording; the value is
    the natural-language wording actually sent to GLiNER2 as its entity-type prompt. 
    One JSON file per NER source, so a different corpus's tagset never requires touching this
    script itself, only pointing --labels-file at a different JSON file."""
    with open(labels_file) as f:
        return json.load(f)


DEFAULT_LABELS_FILE = Path(__file__).parent / "labels.json"
_default_label_map = load_label_map(DEFAULT_LABELS_FILE)
# Fixed output scheme (matches HIPE's NE-COARSE-LIT bare types) -- what
# predicted_entity_type is stored as in ner_features.csv, regardless of the prompt
# wording sent to the model. Downstream consumers (train_b3_logistic_regression.py,
# analyze_ner_mismatches.py, phase2/vocab.py's ENTITY_TYPE_VOCAB, ...) import this
# constant directly, so it stays a plain module-level list -- sourced from labels.json,
# not duplicated.
LABELS = list(_default_label_map.keys())

# Default wording sent to GLiNER2 as its entity-type prompt, one per LABELS entry in the
# same order -- overridable via --labels-file (a different JSON file entirely).
DEFAULT_PROMPT_LABELS = list(_default_label_map.values())
...
def report_chunk_splits(chunks_per_text: list[list[TextChunk]], sentence_keys: list[tuple] | None) -> None:
    """Print which sentences got split by chunk_long_text, and the exact word each split
    landed between -- since chunks are non-overlapping (see chunk_long_text), any entity
    whose words fall on both sides of one of these boundaries is invisible to GLiNER2 in
    either chunk and will be missed or truncated. sentence_keys (parallel to
    chunks_per_text): (document_id, sentence_id) per text, for identifying which sentence
    each split report line refers to; None just numbers them positionally."""
    chunked = [(i, chunks) for i, chunks in enumerate(chunks_per_text) if len(chunks) > 1]
    if not chunked:
        return

    print(
        f"{len(chunked)} sentence(s) exceeded the chunk word limit and were split into "
        "non-overlapping chunks -- entities whose words fall on both sides of a split "
        "below may be missed or truncated:"
    )
    for i, chunks in chunked:
        label = f"{sentence_keys[i][0]} sentence {sentence_keys[i][1]}" if sentence_keys else f"text #{i}"
        print(f"  {label}: split into {len(chunks)} chunks")
        for j in range(len(chunks) - 1):
            words_before = _WORD_PATTERN.findall(chunks[j].text)
            words_after = _WORD_PATTERN.findall(chunks[j + 1].text)
            last_word = words_before[-1] if words_before else "<empty>"
            first_word = words_after[0] if words_after else "<empty>"
            print(f"    split {j + 1}/{len(chunks) - 1}: last token of chunk {j} = {last_word!r}, first token of chunk {j + 1} = {first_word!r}")
...
def extract_candidates_long(
    ...
    if the text was longer than chunk_size words and had to be split. sentence_keys:
    optional (document_id, sentence_id) per text, parallel to texts, used only to label
    the chunk-split report (see report_chunk_splits) -- pass None to number them
...
def build_candidates(
    ...
    prompt_to_hipe: dict[str, str],
) -> pd.DataFrame:
    texts = sentences_df["sentence_text"].tolist()
    sentence_keys = list(zip(sentences_df["document_id"], sentences_df["sentence_id"]))
    official = extract_candidates_long(extractor, texts, prompt_labels, threshold, batch_size, chunk_size, sentence_keys)
                ...
                records.append(
                    {
                        ...
                        "predicted_entity_type": prompt_to_hipe[label],
...
def main():
    ...
    parser.add_argument(
        "--load-data",
        default=str(DEFAULT_LOAD_DATA),
        help="Token-level data CSV produced by preprocessing_data.py (every split, filtered internally as needed)",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "JSON file of {TYPE: prompt wording} -- TYPE is what's written to predicted_entity_type, "
            "the value is the natural-language wording sent to GLiNER2 as its entity-type prompt (see "
            "load_label_map()'s docstring). Default: src/ner/gliner/labels.json (HIPE-2022's 5-type scheme). "
            "Point this at a different JSON file for a NER source with its own tagset, "
            "e.g. data/data_source/letemps/letemps_fr_labels.json."
        ),
    )
    ...
    prompt_labels = list(label_map.values())
    prompt_to_hipe = dict(zip(prompt_labels, labels))
    print(f"Loaded {args.labels_file} -- prompt labels -> output labels: {prompt_to_hipe}")
