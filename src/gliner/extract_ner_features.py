...
written to predicted_entity_type in ner_features.csv. Downstream files never see the
prompt wording -- only the HIPE scheme.
...
Usage:
    ...
    python src/gliner/extract_ner_features.py
    python src/gliner/extract_ner_features.py --limit 50 --out /tmp/smoke_test.csv  # quick smoke test
    python src/gliner/extract_ner_features.py --prompt-labels PERS,LOC,ORG,TIME,PROD  # abbreviated prompt wording instead of the default full words
"""
...
# analyze_gliner_mismatches.py, ...) rely on exactly these five values.
LABELS = ["PERS", "LOC", "ORG", "TIME", "PROD"]

# Default wording sent to GLiNER2 as its entity-type prompt, one per LABELS entry in the
# same order -- overridable via --prompt-labels.
DEFAULT_PROMPT_LABELS = ["person", "location", "organization", "time", "production"]
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Comma-separated labels sent to GLiNER2 as its entity-type prompt, one per LABELS entry "
            f"({', '.join(LABELS)}) in that order -- e.g. 'person,location,organization,time,product' maps "
            "GLiNER2's 'person' output to 'PERS' in predicted_entity_type "
            f"(default: {','.join(DEFAULT_PROMPT_LABELS)})"
        ),
    )
    args = parser.parse_args()

    prompt_labels = [p.strip() for p in args.prompt_labels.split(",")]
    if len(prompt_labels) != len(LABELS):
        parser.error(f"--prompt-labels must have exactly {len(LABELS)} comma-separated entries (one per {LABELS}), got {prompt_labels}")
    prompt_to_hipe = dict(zip(prompt_labels, LABELS))
    print(f"Prompt labels -> HIPE labels: {prompt_to_hipe}")
