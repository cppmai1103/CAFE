"""Convert deduplicate_ner_features.csv (span-level GLiNER2 candidates, already resolved
...
no best-candidate-per-token pick is needed here anymore. Candidates below --threshold are
dropped first, so a token with no sufficiently-confident candidate over it gets "O" in the
Gliner column, instead of always being tagged with GLiNER's best (possibly
...
Usage:
    python src/gliner/ner_features_to_token_format.py
    python src/gliner/ner_features_to_token_format.py --threshold 0.5
    python src/gliner/ner_features_to_token_format.py --limit 5 --out /tmp/smoke_token_format.csv  # quick smoke test
"""
...
def main():
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="Token-level output CSV path (default: hipe2020_train_fr_gliner_token_format_threshold<threshold>.csv)",
    )
    parser.add_argument(
    ...
    print("=== Step 3: Filter by threshold and build Gliner tags ===")
