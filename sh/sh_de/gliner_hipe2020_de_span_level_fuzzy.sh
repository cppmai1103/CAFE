...
################################################################################
## hipe2020_de / gliner, span_level_fuzzy ONLY, IN-DOMAIN -- hipe2020_de has no
## German cross-dataset partner (letemps_fr is French), so unlike
## historical_ner_span_level_fuzzy.sh's fr+letemps pair there's no cross-dataset
## block here: everything below fits/trains on hipe2020_de's own train split,
## early-stops on its own val split, and evaluates on its own test split only.
##
...
##             (src/phase2/expert/)
##
## gliner's raw NER candidates (ner_features/deduplicate_ner_features/label_reliability_*.csv)
## already exist on disk for hipe2020_de (built by data_prepare.sh), so this starts from
## Phase 1 manual feature extraction. gliner's hipe2020_de extraction used the full 5-type
## HIPE-2022 tagset (PERS/LOC/ORG/TIME/PROD), same as hipe2020_fr -- so facet-by-type
## below lists all 5 types.
################################################################################
...
NER_BASE=data/hipe2020_de/gliner/data_baseline
...
PHASE1_FIGS=figures/hipe2020_de/gliner/modeling/level_ablation/$LEVEL/$TRAIN_TEST_TAG
PHASE1_CKPT=checkpoints/hipe2020_de/gliner/phase1/level_ablation/$LEVEL
PHASE2_OUT=data/hipe2020_de/gliner/data_phase2/level_ablation/$LEVEL
PHASE2_FIGS=figures/hipe2020_de/gliner/phase2/level_ablation/$LEVEL/$TRAIN_TEST_TAG
PHASE2_CKPT=checkpoints/hipe2020_de/gliner/phase2/level_ablation/$LEVEL
WINDOWS=$PHASE2_OUT/phase2_candidate_windows.jsonl

VARIANTS_CKPT=checkpoints/hipe2020_de/gliner/phase2/variants_ablation/$TRAIN_TEST_TAG
VARIANTS_OUT=data/hipe2020_de/gliner/data_phase2/variants_ablation/$TRAIN_TEST_TAG
VARIANTS_FIGS=figures/hipe2020_de/gliner/phase2/variants_ablation/$TRAIN_TEST_TAG
